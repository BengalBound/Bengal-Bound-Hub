...
def pipeline_health_monitor():
    ...
    try:
        catalog = AgentCatalog.objects.get(slug='kai')
    ...
    for instance in AgentInstance.objects.filter(catalog=catalog, status='idle'):
        failing = Pipeline.objects.filter(business=instance.business, last_status="failing")
        for pipeline in failing:
            try:
                result = engine.pipeline_health_check(pipeline, instance=instance)
...
def auto_analyze_open_incidents():
    ...
    for instance in AgentInstance.objects.filter(catalog=catalog, status='idle'):
        open_incidents = Incident.objects.filter(business=instance.business, status="open", ai_root_cause="")
        for incident in open_incidents:
            try:
                result = engine.analyze_incident(incident, instance=instance)
