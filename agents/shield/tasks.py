...
def auto_resolve_open_tickets():
    ...
    engine = ShieldEngine()
    open_tickets = ITTicket.objects.filter(status="open", priority__in=["low", "medium"])
    resolved = 0
    ...
    for ticket in open_tickets:
        try:
            ...
            result = engine.resolve_ticket(ticket, kb_articles)
...
def sla_breach_monitor():
    ...
    engine = ShieldEngine()
    open_tickets = ITTicket.objects.filter(status__in=["open", "ai_resolving"], sla_breached=False)
    breached_count = 0

    for ticket in open_tickets:
        sla = engine.sla_assessment(ticket)
        if sla["breached"]:
